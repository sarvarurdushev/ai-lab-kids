...
to a paid plan. For an app teachers keep using every class, that's not
usable as-is. Instead, this uses [Neon](https://neon.com), whose free
Postgres tier is permanent (not a trial) and doesn't require a card.

## First deploy

1. **Create a free Neon database.** Sign up at [neon.com](https://neon.com)
   (no credit card), create a project, and copy its connection string
   (`postgresql://...`). Neon gives you one connection string that works for
   both pooled and direct connections — use the same value for both env vars
   below.
...
3. In the Render dashboard: **New → Blueprint**, point it at this repo.
   ...
   will prompt you to paste in a value for each — paste your Neon connection
   string into both.
4. Apply the blueprint. The web service is created on the **Free** instance
   ...
   against your Neon database), then the server starts.
...
- **Neon free Postgres**: permanent, not time-limited. 0.5 GB storage,
  100 compute-hours/month, 5 GB network egress/month. Compute also
  auto-suspends when idle and wakes on the next query (similar cold-start
  tradeoff to the web service). Comfortably enough for a small
  school/classroom's worth of usage.
...
|---|---|---|
| `DATABASE_URL` / `DIRECT_DATABASE_URL` | you paste your Neon connection string in during blueprint setup (`sync: false`) | same value for both |
...
Neon connection string copied into `.env.local`:
